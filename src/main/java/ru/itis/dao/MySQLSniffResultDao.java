...
public class MySQLSniffResultDao extends MySqlDao implements SniffResultDao {
    ...
    public void create(SniffResult sniffResult) {
        ...
        try {
            stmt = con.prepareStatement("INSERT INTO sniffresult (referer, hijackedurl, author, sniffdate) VALUES( ?,?,?,?)");
            stmt.setString(1, sniffResult.getReferer());
            stmt.setString(2, sniffResult.getHijackedUrl());
            stmt.setString(3, sniffResult.getAuthor());
            stmt.setTimestamp(4, new java.sql.Timestamp(sniffResult.getSniffingDate().getTime()));
            stmt.execute();
    ...
    public SniffResult readSniffResult(SniffResult sniffResult) {
        ...
        try {
            ...
            sniffResult1.setAuthor(rs.getString("author"));
    ...
    public List<SniffResult> readAllSniffResults() {
        ...
        try {
            ...
            while (rs.next()
                ...
                sniffResult.setAuthor(rs.getString("author"));
    ...
    @Override
    public List<SniffResult> readHackerSniffResult(String hacker) {
        String sql = "SELECT * FROM sniffresult WHERE author = ?";
        List<SniffResult> sniffResultList = new ArrayList<SniffResult>();
        PreparedStatement stm = null;
        Connection con = getConnection();
        try {
            stm = con.prepareStatement(sql);
            stm.setString(1, hacker);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                SniffResult sniffResult = new SniffResult();
                sniffResult.setId(rs.getLong("id"));
    ...
    @Override
    public String readRole(String name) {
